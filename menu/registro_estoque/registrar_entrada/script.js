...
addMp.addEventListener("click", function () {
	...
	row.innerHTML = `
        ...
        <td>
            R$ ${(parseFloat(custo) || 0).toLocaleString("pt-BR", {
...
function atualizarCustoTotalGeral() {
	...
	totalRow.innerHTML = `
        ...
        <td><input type="text" id="custo-total-geral-input" value="${total.toFixed(
			2
...
form.addEventListener("submit", function (e) {
	...
	if (!confirm("Você tem certeza que deseja registrar a entrada?")) {
		e.preventDefault();
	}
});
